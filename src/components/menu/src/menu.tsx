import { defineComponent, h, useAttrs, resolveComponent, toRefs } from "vue";
...
export default defineComponent({
  ...
  setup(props, ctx) {
    const renderMenu = (data: RouterType) => {
      return data.map((item) => {
        ...
        const slots = {
          title: () => (
            <>
              <el-icon size={14}>{createIcon(iconName)}</el-icon>
        ...
        return (
          <el-menu-item index={item.path}>
            <el-icon size={25}>{createIcon(iconName)}</el-icon>
            <span>{item.meta.label}</span>
          </el-menu-item>
        );
      });
    ...
    return () => {
      const { defaultActive, router, data } = toRefs(props);
      return (
        <el-menu
